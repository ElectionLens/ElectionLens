#!/usr/bin/env python3
"""
Fix TN-033 (Thiruporur) by directly setting booth votes to match official totals.
Then set postal to 0 (since booth = official).
"""
...
def fix_thiruporur_direct():
    """Fix TN-033 by directly setting booth votes to official totals."""
    ac_id = 'TN-033'
    ...
    for i, cand, official_cand in candidate_matches:
        if official_cand:
            ...
            if current_booth > 0:
                # Distribute official votes proportionally based on current booth distribution
                # Calculate proportion for each booth
                proportions = []
                for r in results.values():
                    votes = r.get('votes', [])
                    if i < len(votes):
                ...
                if remainder != 0:
                    # Add remainder to booths with highest proportions
    ...
    # Build postal candidates (all postal = 0 since booth = official)
    postal_candidates = []
...
if __name__ == "__main__":
    print("=" * 70)
    print("Fixing TN-033 (Thiruporur) - Direct Approach")
    print("=" * 70)
    
    result, msg = fix_thiruporur_direct()
