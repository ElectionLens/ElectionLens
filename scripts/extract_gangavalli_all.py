#!/usr/bin/env python3
"""Extract all Gangavalli booths with correct column mapping."""
...
COLUMN_MAPPING = {
    4: 1,   # ADMK (KUMARAGURU R) - 72235
    5: 0,   # DMK (MALAIYARASAN D) - 77483
    6: 6,   # BSP (JEEVANRAJ N) - 403
    7: 3,   # PMK (DEVADASS RAMASAMY) - 11108
    8: 4,   # IND (PRABU C) - 404
    9: 5,   # IND (BALAKRISHNAN P) - 95
    10: 13, # NADLMMKLK (VENKATRAMAN J) - 77
    11: 2,  # NTK (JAGADESAN A) - 10612
    12: 11, # IND (SUBRAMANIAN R K) - 135
    13: 14, # IND (KAMALAKANNAN M) - 94
    14: 9,  # IND (KUMARAGURU N) - 419
    15: 16, # IND (GOVINDARAJ S R) - 110
    16: 17, # IND (SIGAMANI A) - 72
    17: 12, # IND (ARUL INIYAN A) - 121
    18: 10, # IND (MURUGESAN M) - 421
    19: 15, # DMSK (PALANIYAMMAL S) - 584
    20: 7,  # IND (MAYILAMPARAI MARI A) - 356
    21: 8,  # AMM (NATTANMAI GUNASEKARANA A.S) - 273
    22: 18, # IND (RAJASEKAR K) - 50
    23: 19, # IND (JAYABAL PM) - 58
    24: 20, # IND (RAJAMANICKAM C) - 72
}

def extract_all_booths():
    """Extract all booths starting from row 4."""
    results = {}
    ...
    with pdfplumber.open(PDF_PATH) as pdf:
        ...
        for page_num, page in enumerate(pdf.pages, 1):
            ...
            for table in tables:
                ...
                # Start from row 4 (row 0-3 are headers)
                for row_idx in range(4, len(table)):
                    ...
                    # Extract booth number (can be in column 0 or 1)
                    booth_no = None
...
def main():
    print("=" * 70)
    print("Extracting All Gangavalli 2024 Booths")
    print("=" * 70)
    ...
    # Extract all booths
    print(f"\n📥 Extracting all booths from PDF...")
    ...
    existing_data['source'] = 'Tamil Nadu CEO - Form 20 (extracted with correct column mapping from AC081.pdf)'
    ...
    print(f"   Total booths: {len(extracted_results)}")
