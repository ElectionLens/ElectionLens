...
def process(ac_id,ac,folder):
 ...
 if not include_nota:
  nota_name=next(c['name'] for c in ac['candidates'] if c.get('party')=='NOTA'); booth[nota_name]=sum(r[3] for r in rows.values())
 diffs={c['name']:c['votes']-booth.get(c['name'],0) for c in ac['candidates']}; residual=sum(diffs.values())
 ...
 return {'status':'ok','rows':len(rows),'raw':rows,'mapping':mapping,'boothSums':booth,'pdf':str(pdfs[0]),'includeNota':include_nota}
...
def build_doc(ac_id,ac,out):
 names=[c['name'] for c in ac['candidates']]; non=[c for c in ac['candidates'] if c.get('party')!='NOTA']; mapped=ac['candidates'] if out.get('includeNota') else non; nota_idx=names.index(next(c['name'] for c in ac['candidates'] if c.get('party')=='NOTA')); results={}
