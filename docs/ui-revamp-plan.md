...
`App.tsx:1369` passes MapView roughly 45 props, including all result-panel state, year
selectors, share URLs, and PC-contribution state. Two consequences:

1. **The result panels are not map-specific.** `ElectionResultPanel`, `YearSelector`, and
   the share affordances currently live under MapView but have nothing to do with
   Leaflet. Card mode needs them too. Hoist them to a shared shell **before** adding the
   toggle, or card mode will be a feature-poor copy.
2. **Conditional unmount is expensive.** `{surface === 'map' ? <MapView/> : <Cards/>}`
   destroys the Leaflet instance and all tile/GeoJSON caches on every toggle. Prefer
   keeping MapView mounted and visually hidden **if** measurement shows remount cost is
   real — but note that hidden-but-mounted Leaflet needs `invalidateSize()` on reveal
   (`MapResizer.tsx` already exists for exactly this). Decide with a measurement, not a
   guess.
